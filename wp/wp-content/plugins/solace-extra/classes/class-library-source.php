 ...
 *
 * @package SolaceExtraElementorAddons
 * @author Solace Extra
 */
...
class Library_Source extends Source_Base {
	 ...
	 */
	const LIBRARY_CACHE_KEY = 'solace_extra_elementor_library_cache';
	 ...
	 */
	const API_TEMPLATES_INFO_URL = SOLACE_EXTRA_API_BASE_URL . 'templates';
	// const API_TEMPLATES_INFO_URL = 'http://localhost/coding/solace/wp-json/solace/v1/templates';

	/**
	 * Template data api url
	 */
	const API_TEMPLATE_DATA_URL = SOLACE_EXTRA_API_BASE_URL . 'template/';
	// const API_TEMPLATE_DATA_URL = 'http://localhost/coding/solace/wp-json/solace/v1/template/';

	public function get_id() {
		return 'solace-extra-library';
	}

	public function get_title() {
		return __( 'Solace Extra Library', 'solace-extra' );
	}
	...
	public function save_item( $template_data ) {
		return new WP_Error( 'invalid_request', 'Cannot save template to a solace library' );
	}

	public function update_item( $new_data ) {
		return new WP_Error( 'invalid_request', 'Cannot update template to a solace library' );
	}

	public function delete_template( $template_id ) {
		return new WP_Error( 'invalid_request', 'Cannot delete template from a solace library' );
	}

	public function export_template( $template_id ) {
		return new WP_Error( 'invalid_request', 'Cannot export template from a solace library' );
	}
	...
	private function prepare_template( array $template_data ) {
		return array(
			...
			'custom'      => ! empty( $template_data['custom'] ) ? $template_data['custom'] : false,
			'preview'     => ! empty( $template_data['preview'] ) ? $template_data['preview'] : '',
		);
	}
	...
	public function get_data( array $args, $context = 'display' ) {
		...
		if ( empty( $data ) || empty( $data['content'] ) ) {
			throw new Exception( esc_html__( 'Template does not have any content', 'solace-extra' ) );
		}
	...
	public static function request_template_data( $template_id ) {
		...
		// if ( did_action( 'solace_extra_elementor_addons_pro_loaded' ) && class_exists( '\SolaceExtraElementorAddonsPro\Libs\Solace_Extra_Elementor_License' ) && 'valid' === Solace_Extra_Elementor_License::$license_activate ) {
		// 	$body['has_pro']     = 1;
		// 	$body['pro_version'] = SOLACE_EXTRA_ELEMENTOR_ADDONS_PRO_VERSION;
		// }
